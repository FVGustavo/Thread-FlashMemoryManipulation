...
void taskGravaFlash(void)
{
  int16_t received_num_3;
  esp_err_t err;
  nvs_handle_t nvs_handle_1;

  while (1)
  {
    xQueueReceive(queue_handle_3, &received_num_3, portMAX_DELAY);
    ...
    err = nvs_set_i16(nvs_handle_1, "th_grava_flash", received_num_3);
    if (err != ESP_OK)
    {
...
void taskReadFlash(void)
{
  esp_err_t err;
  nvs_handle_t handle;
  int16_t stored_value_1[QUEUE_SIZE] = {0};
  ...
  while (1)
  {
    ...
    err = nvs_get_i16(handle, "th_grava_flash", &stored_value_1);
    if (err == ESP_ERR_NVS_NOT_FOUND)
    {
      printf("Nenhum valor gravado \n");
    }
    if (err == ESP_OK)
    {
