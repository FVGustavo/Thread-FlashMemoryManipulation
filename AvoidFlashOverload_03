  ...
  while (1)
  {
    ...
    if (xSemaphoreTake(mutex, portMAX_DELAY) == pdTRUE)
    {
      ...
      if (received_num_1 < MIN_1 || received_num_1 > MAX_1)
      {
        ...
        err = nvs_set_i16(nvs_handle_1, "th_read_num_1", received_num_1);
        if (err != ESP_OK)
        {
  ...
  while (1)
  {
    ...
    if (xSemaphoreTake(mutex, portMAX_DELAY) == pdTRUE)
    {
      ...
      if (received_num_2 < MIN_2 || received_num_2 > MAX_2)
      {
        ...
        err = nvs_set_i16(nvs_handle_2, "th_read_num_2", received_num_2);
        if (err != ESP_OK)
        {
          printf("Erro ao gravar na flash \n");
          return;
...
void taskReadNVS(void)
{
  esp_err_t err;
  nvs_handle_t handle;
  int16_t stored_value_1 = 0;
  int16_t stored_value_2 = 0;
  ...
  err = nvs_get_i16(handle, "th_read_num_1", &stored_value_1);
  if (err == ESP_ERR_NVS_NOT_FOUND)
  {
    printf("Valor da task 1 não inicializado \n");
  }
  else if (err == ESP_OK)
  {
    printf("Valor lido da task 1: %hd\n", stored_value_1);
  }
  else
  {
    printf("Erro na leitura da task 1\n");
  }

  err = nvs_get_i16(handle, "th_read_num_2", &stored_value_2);
  if (err == ESP_ERR_NVS_NOT_FOUND)
  {
    printf("Valor da task 2 não inicializado \n");
  }
  else if (err == ESP_OK)
  {
    printf("Valor lido da task 2: %hd\n", stored_value_2);
  }
  else
  {
    printf("Erro na leitura da task 2 \n");
  }
