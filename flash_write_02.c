...
void read_nvs_value(void)
{
  ...
  if (err != ESP_OK)
  {
    printf("Erro ao abrir o handle \n");
    return;
  }

  err = nvs_get_i32(my_handle, "th_read_num_1", &stored_value);
  if (err == ESP_ERR_NVS_NOT_FOUND)
  {
    printf("O valor ainda não foi inicializado \n");
  }
  else if (err == ESP_OK)
  {
    printf("Valor atual gravado: %d \n", stored_value);
  }
  else
  {
...
void taskReadNum2(void *pvParameter)
{
  ...
  while (1)
  {
    ...
    if (received_num_2 < -200 || received_num_2 > 1000)
    {
      gpio_set_level(LED1_PIN, 0);
      vTaskDelay(pdMS_TO_TICKS(1000));
