...
void taskReadNVS(void *pvParameter)
{
  esp_err_t err;
  ...
  while (1)
  {
    ...
    err = nvs_get_i32(my_handle, "th_read_num_1", &stored_value_1);
    ...
    {
      printf("Valor atual th_read_num_1: %ld \n", stored_value_1);
    }
    else
    {
    ...
    err = nvs_get_i32(my_handle, "th_read_num_2", &stored_value_2);
    if (err == ESP_ERR_NVS_NOT_FOUND)
    {
    ...
    {
      printf("Valor atual th_read_num_2: %ld \n", stored_value_2);
    }
    else
    {
...
void app_main(void)
{
  ...
  th_read_nvs = xTaskCreate((TaskFunction_t)taskReadNVS, "thread 5", 2 * 1024, NULL, 5, &th_handle_5);
  ...
  if (th_read_nvs != pdPASS)
  {
    printf("Erro ao criar thread 5 \n");
    return;
  }
